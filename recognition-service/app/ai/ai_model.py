...
params = Parameters('./model/best.pt')
...
def load_yolov5_model():
    ...
    :return: model, names
    """
    model = attempt_load(params.model, device=params.device)
...
def preprocess_image(frame):
    """
    Apply preprocessing steps to the input frame for better detection.
...
def detection(frame, model, names):
    """
    Takes an image, runs it through the model, and returns the image with bounding boxes drawn around
    ...
    label = ""
    cropped_detected_plate = None
    # detections per image
    for i, det in enumerate(pred):

        img_shape = frame.shape[2:]
        out_shape = detected_plate.shape
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if x1 < 0 or y1 < 0 or x2 > detected_plate.shape[1] or y2 > detected_plate.shape[0]:
                    continue
                ...
                cropped_detected_plate = detected_plate[y1:y2, x1:x2].copy()
                ...
                cv2.rectangle(
                    detected_plate, c1, c2, params.color, thickness=tl, lineType=cv2.LINE_AA
                )
                ...
                if label:
                    ...
                    cv2.rectangle(detected_plate, c1, c2, params.color, -1, cv2.LINE_AA)  # filled
                    cv2.putText(
                        detected_plate,
                        label,
                        (c1[0], c1[1] - 2),
    ...
    return detected_plate, cropped_detected_plate